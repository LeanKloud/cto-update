...
import { ChevronDown, ArrowUp, HardDrive } from 'lucide-react';
...
const StorageUnattachedDisks: React.FC = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      // Filter for unattached disks only
      const unattachedDisks = data.filter(item => 
        item.storage_type.toLowerCase().includes('unattached') || 
        item.host_asset === 'unattached' ||
        !item.host_asset
      );
      
      setRecommendations(unattachedDisks);
      ...
      const mockRecommendations: StorageRecommendation[] = [
        {
          ...
          recommendations: {
            ...
            object: {
              ...
              cost_savings: 2.00,
              description: 'Archive unattached data to S3 Glacier for long-term storage'
            },
            snapshot: {
              snapshot_type: 'gp2',
              cost_savings: 0.50,
              description: 'Create snapshot before deletion to preserve data'
            }
          }
        }
      ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-white mb-2">Unattached Disks</h1>
              <p className="text-slate-300">Identify unattached EBS volumes and optimize storage spend</p>
            </div>
            <button
              onClick={() => {
                setLoading(true);
        ...
        <div className="bg-slate-800 rounded-xl shadow-2xl border border-slate-700 overflow-hidden">
          {loading ? (
            <div className="p-12 text-center">
              ...
              <div className="mt-4 text-slate-300">Loading unattached disks...</div>
            </div>
          ) : error ? (
            <div className="p-12 text-center">
            ...
            <div className="p-12 text-center">
              <div className="text-gray-500 text-lg mb-2">No unattached disks found</div>
              <div className="text-gray-400 text-sm">All your storage volumes are properly attached</div>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full" role="table">
                <thead className="bg-slate-700 border-b border-slate-600">
                  <tr>
                    <th className="text-left py-4 px-6 font-medium text-white" scope="col">Storage Id</th>
                    ...
                    return (
                      <React.Fragment key={item.storage_id}>
                        ...
                        >
                          ...
                          <td className="py-4 px-6">
                            <span className="text-white">{item.throughput_idle_pct}%</span>
                          </td>
                          <td className="py-4 px-6">
          ...
          <div className="mt-8 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-slate-700 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-white mb-2">Block Recommendation</h4>
                <p className="text-slate-300 text-sm">No specific block storage recommendations available</p>
              </div>
              <div className="bg-slate-700 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-white mb-2">Object Recommendation</h4>
                <p className="text-slate-300 text-sm">No object storage recommendations available</p>
              </div>
              <div className="bg-slate-700 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-white mb-2">Snapshot Recommendation</h4>
                <p className="text-slate-300 text-sm">No snapshot recommendations available</p>
              </div>
            </div>
          </div>
        )}
...
export default StorageUnattachedDisks;
